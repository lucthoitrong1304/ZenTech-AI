...
logger = logging.getLogger("ai-service.product-search")
...
def filter_explicit_product_matches(query: str, candidates: list[dict]) -> list[dict]:
    ...
    filtered = [candidate for name, candidate in matches if len(name) == longest_length]
    logger.info("Filtered explicit product matches: candidates=%s matches=%s", len(candidates), len(filtered))
    return filtered
...
def search_product_candidates(query: str, limit: int = 5, category_name: str | None = None) -> list[dict]:
    ...
    if not clean_query:
        logger.warning("Product search skipped because query is empty: collection=%s", col_name)
        return []
    if not settings.qdrant_url or not settings.embedding_deployment_name:
        logger.warning("Product search skipped because Qdrant URL or embedding deployment is not configured: collection=%s", col_name)
        return []

    logger.info(
        "Starting product search: collection=%s limit=%s category=%s query_preview='%s'",
        col_name,
        limit,
        category_name,
    ...
    try:
        ...
        logger.error("Error checking Qdrant product collection: collection=%s", col_name, exc_info=True)
        return []
    ...
    if norm_cat:
        logger.info("Filtering product search by category: category=%s", norm_cat)
    ...
    try:
        ...
        if candidates:
            logger.info("Product search completed: candidates=%s", len(candidates))
        else:
            logger.warning("No suitable product candidates found: collection=%s", col_name)
        return candidates

    except Exception:
        logger.error("Failed to query product collection from Qdrant: collection=%s", col_name, exc_info=True)
        return []
